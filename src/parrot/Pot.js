...
const RetrieveWateringStatusTask = require('./RetrieveWateringStatusTask');
const WriteLedStateTask = require('./WriteLedStateTask');

const FLOWER_POT_LIVE_SERVICE_UUID = '39e1f90084a811e2afba0002a5d5c51b';

/** 
 * Refresh the battery level every 36 hours. Given that it lasts 180 days, this
...
/**
 * Refresh the watering status once every 24h?
 */
const WATERING_STATUS_REFRESH_INTERVAL = 24 * 60 * 60 * 1000;
...
function isPotAdvertisement(advertisement) {
  return advertisement.serviceUuids && advertisement.serviceUuids.includes(FLOWER_POT_LIVE_SERVICE_UUID);
}
...
class FlowerPotDevice extends Device {
  ...
  async _initialize() {
    ...
    await this.requestWateringStatus();
  }
  ...
  async requestWateringStatus() {
    debug('Retrieving sensor data');
    try {
      const task = new RetrieveWateringStatusTask();
      const wateringStatus = await this.execute(task);

      wateringStatus.timestamp = Date.now();

      debug(`Retrieved watering status data: ${util.inspect(wateringStatus)}`);
      this.emit('wateringStatus', wateringStatus);
    }
    catch (e) {
      debug(`Failed to retrieve the flower power data. Error: ${util.inspect(e)}`);
    }

    this._scheduleWateringStatusRefresh();
  }

  _scheduleWateringStatusRefresh() {
    if (this._wateringStatusTimer) {
      clearTimeout(this._wateringStatusTimer);
    }

    this._wateringStatusTimer = setTimeout(this.requestWateringStatus.bind(this), WATERING_STATUS_REFRESH_INTERVAL);
  }
...
async function createDevice(executor, peripheral) {
  const device = new FlowerPotDevice(executor, peripheral);
  await device._initialize();
  return device;
}

module.exports = {
  isPotAdvertisement: isPotAdvertisement,
